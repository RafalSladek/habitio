const { test: wrapperTest } = require("@playwright/test");

wrapperTest.describe.skip("legacy monolithic suite replaced by split specs", () => {
  // @ts-check
  ...
  async function seedHabit(
    ...
    await page.evaluate(
      (/** @type {{daysOld: number, checkedDaysBack: number}} */ { daysOld, checkedDaysBack }) => {
  ...
  test.describe("habit.io", () => {
    ...
    test.describe("after onboarding", () => {
      ...
      test("add habit from suggestions list", async ({ page }) => {
        await page.locator("#fab-add").click();
        await page
          .locator(".suggestion-item", { hasText: "Drink 2L Water" })
          .getByText("+")
          .click();
    ...
    test.describe("GA4 event tracking", () => {
          ...
          function installSpy(/** @type {any} */ arr) {
            arr.push = function () {
              ...
              if (item && typeof item === "object") {
                try {
                  /** @type {any} */ (window).__gtagCalls.push(
        ...
        const navPageViews = calls
          ...
          .filter(
            (c) =>
              ...
              c[1] === "page_view" &&
              c[2] &&
              typeof c[2].page_title === "string" &&
              c[2].page_title.includes("Journal")
          );
      ...
      test("user properties set with age_group, sex, ui_language", async ({ page }) => {
        ...
        const userPropCall = calls.find(
          (c) => c[0] === "set" && c[1] === "user_properties" && c[2] && c[2].age_group === "young"
        );
      ...
      test("habit_complete event fired when a habit is checked", async ({ page }) => {
        ...
        const completeEvent = calls
          .slice(before)
          .find((c) => c[0] === "event" && c[1] === "habit_complete");
    ...
    test.describe("export / import", () => {
      ...
      test("export downloads a JSON file containing current habits and checks", async ({
        page,
      }) => {
      ...
      test("import merges new habits and skips duplicates", async ({ page }) => {
        ...
        const state = await page.evaluate(() =>
          JSON.parse(localStorage.getItem("habitio_v5") || "{}")
        );
        ...
        expect(
          state.habits.filter((/** @type {any} */ h) => h.name === "Drink Water")
        ).toHaveLength(1);
      });
