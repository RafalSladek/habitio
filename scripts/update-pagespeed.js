...
for (const run of runs) {
  try {
    execSync(
      `gh run download ${run.databaseId} --name lighthouse-results --dir "${TMP}"`,
      { stdio: "pipe" }
    );
...
const newRow =
  `| ${ts}    | ${version.padEnd(7)} | ${title.padEnd(31)} | ${String(avg("perf")).padStart(4)} | ${String(avg("a11y")).padStart(4)} | ${String(avg("bp")).padStart(14)} | ${String(avg("seo")).padStart(3)} |`;
